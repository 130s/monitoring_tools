...
class MonitoredTopic:
    ...
    def topic_data_callback(self, msg, logger_=logger):
        # TODO logger becomes error for ROS1
...
class TopicMonitor:
    ...
    def main(topic_monitor_class, datarecv_thread_class):
        ...
        @summary: Method to take console input and instantiate the meat of
            the derived class.
        @param topic_monitor_class: Derived class of
            'topic_monitor.TopicMonitor'
        @param datarecv_thread_class: Derived class of
            'topic_monitor.AbstDataReceivingThread'
        """
    ...
    @abstractmethod
    def run_topic_listening(self, topic_monitor, options, node):
        """
        @summary: Subscribe to relevant topics and manage the data received from susbcriptions.
        """
...
class AbstDataReceivingThread(Thread):

    @abstractmethod
    def __init__(self, topic_monitor, options):
        """
        @summary: TBD
        @param topic_monitor: Derived class of 'topic_monitor.AbstDataReceivingThread'        
        """
        super(AbstDataReceivingThread, self).__init__()
        raise NotImplementedError()
